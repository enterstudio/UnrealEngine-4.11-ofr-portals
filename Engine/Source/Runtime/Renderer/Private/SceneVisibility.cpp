...
struct FRelevancePacket
{
    ...
    FRelevancePrimSet<FPrimitiveSceneProxy*> PortalPrimSet;
	...
	void ComputeRelevance()
	{
		...
		for (int32 Index = 0; Index < Input.NumPrims; Index++)
		{
			...
			{
				// Add to set of custom depth primitives
				CustomDepthSet.AddPrim(PrimitiveSceneInfo->Proxy);
			}
            
            if (ViewRelevance.bRenderPortal)
            {
                // Add to set of portal primitives
                PortalPrimSet.AddPrim(PrimitiveSceneInfo->Proxy);
            }
	...
	void RenderThreadFinalize()
	{
        ...
        PortalPrimSet.AppendTo(WriteView.PortalPrimSet);
