...
void FSceneRenderer::RenderPortalPass(FRHICommandListImmediate& RHICmdList)
{
    // do we have primitives in this pass?
    bool bPrimitives = false;
    ...
    if (!Scene->World || (Scene->World->WorldType != EWorldType::Preview && Scene->World->WorldType != EWorldType::Inactive))
    {
        for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("PortalPrimset.NumPrims() = %d"), View.PortalPrimSet.NumPrims());
            
            if (View.PortalPrimSet.NumPrims())
            {
                bPrimitives = true;
                break;
            }
    ...
    SCOPED_DRAW_EVENT(RHICmdList, Portals);
    ...
    for (int32 ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
    {
        ...
        // Render the portal primatives into the stencil buffer
		SCOPED_DRAW_EVENT(RHICmdList, StencilPass);
        View.PortalPrimSet.DrawPrims(RHICmdList, View);
		...
		FOneColorPS* PixelShader = *MRTPixelShader;

		SetGlobalBoundShaderState(RHICmdList, ViewFamily.GetFeatureLevel(), BoundShaderState, GetVertexDeclarationFVector4(), *VertexShader, PixelShader);

		FLinearColor ClearColors[1];
		ClearColors[0] = FLinearColor(0, 0, 0, 0);
		PixelShader->SetColors(RHICmdList, ClearColors, 1);

		FVector4 Vertices[4];
		Vertices[0].Set(-1.0f,  1.0f, 0.0f, 1.0f);
		Vertices[1].Set( 1.0f,  1.0f, 0.0f, 1.0f);
		Vertices[2].Set(-1.0f, -1.0f, 0.0f, 1.0f);
		Vertices[3].Set( 1.0f, -1.0f, 0.0f, 1.0f);
